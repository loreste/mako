...
> **Status note (2026-08-08, branch `selfhosted` @ `4fa1de4`):** the branch
> integration this plan prepares is already done — `main` @ `f638e64` is fully
> merged into `selfhosted` (merge-base = tip of `main`), so no
> `integrate/mako-only-selfhost` branch is needed. The CI/Makefile/gate-script
> requirements in "Porting rules" 1–2 and 5–7 are verified present. Remaining
> parity gaps and the live self-host frontier are tracked in
> [SELFHOST_CRITICAL_PATH.md](SELFHOST_CRITICAL_PATH.md) and
> [SELF_HOSTING_PROGRESS.md](SELF_HOSTING_PROGRESS.md); this document remains
> the governing migration contract.
