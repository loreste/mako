...
`makori version` (also `mako --version` / `-V`) prints `makori version mako0.6.33 darwin/arm64`. Use `makori version -v` for an optional commit line.
...
```mko
// Ownership (0.4.0): free at scope exit; views never free
...
### User generics (0.2.2) — monomorphized
