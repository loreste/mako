...
Last inventory: **2026-09-25** · product **mako 0.6.38** (tip; release tag **v0.6.38**) ·
...
const-fn depth (match/while/for/strings · `s[i]`) · **477** `examples/testing`
`*_test.mko` files · **2026-09-25:** release claims gate passed · `cargo test` **450 passed, 0 failed** ·
...
| Scope | Approx. |
|-------|---------|
| **Product version** | **0.6.38** tip · release tag **v0.6.38** ([ROADMAP.md](ROADMAP.md), [VERSIONING.md](VERSIONING.md)) |
...
| iterator invalidation / channel trylock (TSan) / 8MB stack / raw array move tracking | Done — 0.6.34 |
...
| `cargo test --release` | PASS — **146** unit tests (2026-08-19) |
...
| Stdlib safety completion | **Passed** (2026-08-26): 144 std files, 102 checked-native packages with package-local evidence, 38 safe, 4 unsafe-boundary explicitly excluded from default-safe claims, 3 enforced contract families |
...
| `chan50k` regression-only budget ≤1.5× Rust | Done (not a speed claim) |
