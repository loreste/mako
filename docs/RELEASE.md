...
**Product version:** **0.4.0** (`Cargo.toml` / `mako version` → `mako0.4.0`).  
**Published release:** [GitHub release `v0.4.0`](https://github.com/loreste/mako/releases/tag/v0.4.0).
The currently published workflow artifacts are platform-specific; this guide
does not promise a binary for every target triple. External: homebrew-core
publish (see STATUS).
...
```bash
curl -fsSL https://github.com/loreste/mako/releases/download/v0.4.0/install-release.sh \
  | bash -s -- --version v0.4.0 --yes
```
