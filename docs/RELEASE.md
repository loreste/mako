...
**Product version:** **0.5.12** (`Cargo.toml` / `mako version`);
last published tag **v0.5.12**.
...
**Train:** **0.5.12** flake and CI honesty: hard gates may not hide failures
behind `continue-on-error`; flaky/platform failures must be fixed or listed in
an explicit quarantine with unknown-failure and recovered-quarantine checks.
