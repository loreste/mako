...
**Product version:** **0.6.3** (`Cargo.toml` / `makori version`);
last published tag **v0.6.3**.
...
**Train:** **0.6.3** durable C-backend loop ownership: per-iteration cleanup,
break/continue cleanup, sanitizer coverage, and long-running RSS gates must
remain green.
