...
**Product version:** **0.5.13** (`Cargo.toml` / `mako version`);
last published tag **v0.5.13**.
**Tree status:** `examples/testing` inventory **421** `*_test.mko` files
(2026-08-24): C **421 passed, 0 failed**; native **421 passed, 0 failed**.
...
**Train:** **0.5.13** performance contract: faster-than-Rust claims are
workload-specific, strict claims are capped at 1.5× Rust, parser hot paths have
CI budgets, and channel send/recv is regression-only until it earns the speed
claim.
...
- Windows full-suite failures are not hidden: `scripts/windows-quarantine-gate.sh`
  compares results against the 20-entry `ci/windows-quarantine.txt`, fails on new failures,
  and fails when quarantined fixtures start passing until the list is cleaned.
