...
**Product version:** **0.4.13** tip (`Cargo.toml` / `mako version` → `mako0.4.13`);
...
**Train:** **0.4.6–0.4.8** residual/modes/gates · **0.4.9** LLVM CI + install smoke · **0.5.0** CLI default flip.  
...
- [x] Native testing corpus green (`examples/testing --backend native` → **367/367**)
- [x] LLVM release path on primary host (Apple arm64; see CHANGELOG 0.4.5 honest table)
- [x] `scripts/package-release.sh --slim` tarball + `.sha256` for host triple
- [x] Install script smoke from local `dist/` (`mako doctor` ok)
- [x] CHANGELOG **0.4.5** section final
- [x] CI: native backend suite on Linux/macOS
- [x] Tag `v0.4.5` from `native-compiler` → `release.yml` multi-OS packages
- [x] Formula + winget SHAs filled (`fill-release-packaging.sh v0.4.5`)
- [x] Merge `native-compiler` → `main`
