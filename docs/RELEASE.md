...
**Product version:** **0.6.2** (`Cargo.toml` / `makori version`);
last published tag **v0.6.2**.
...
**Train:** **0.6.2** copy-on-write slice ownership: allocator pairing,
retain/release balance, native call/return transfer, sanitizers, differential
tests, and long-running RSS gates must remain green.
