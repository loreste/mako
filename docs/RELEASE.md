...
**Product version:** **0.5.14** (`Cargo.toml` / `mako version`);
last published tag **v0.5.14**.
**Tree status:** `examples/testing` inventory **422** `*_test.mko` files
(2026-08-25): C **422 passed, 0 failed**; native **422 passed, 0 failed**.
...
**Train:** **0.5.14** stdlib safety completion: every checked-native package
must have package-local safety evidence, unsafe-boundary packages are excluded
from default-safe claims, and docs state what is safe, checked, unsafe, or
external.
...
- `scripts/stdlib-safety-audit.sh` now fails releases when any checked-native
  stdlib package lacks package-local safety evidence.
- Unsafe-boundary packages must carry explicit excluded-from-default-safe
  evidence.
- `docs/STDLIB_SAFETY.md`, `docs/STDLIB_SAFETY_MATRIX.md`, and
  `docs/STDLIB_SAFETY_CONTRACTS.md` define the exact default-safe claim.
