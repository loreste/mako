...
**Product version:** **0.5.15** (`Cargo.toml` / `mako version`);
...
- `mako fmt --check` is available for CI/editor gates and fails when files
  differ from canonical formatting.
- `mako doctor` validates native runtime sources plus structured
...
- Native backend hardening: `mako test examples/testing --backend native`
...
- **Native-first default**: `mako build` / `run` / `test` now use the native
...
- `mako pkg get <name>` works out of the box — no config needed
...
- Remote HTTPS package registry (`mako pkg get`, `MAKO_REGISTRY_URL`)
- Ed25519 package signing (`mako pkg sign` / `mako pkg verify`)
