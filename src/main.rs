...
fn compile_to_ast_with(file: &Path, incr: &incremental::IncrOptions) -> Result<ast::Program, ()> {
    ...
    let mut program = tooling::merge_package_dir_siblings(file, program).map_err(|e| {
        Diagnostic::error(&path, &src, Span::unknown(), e).emit();
    })?;
    // Re-run if-let/embed desugar on merged program (imported/sibling files may have IfLet).
