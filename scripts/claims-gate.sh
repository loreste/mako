...
# Local evidence gate for product claims described in public docs.
...
if ! command -v rg >/dev/null 2>&1; then
  echo "claims-gate: ripgrep (rg) required" >&2
  exit 1
fi

if git -C "$ROOT" ls-files --error-unmatch docs/CLAIMS.md >/dev/null 2>&1; then
  echo "claims-gate: docs/CLAIMS.md must remain local untracked" >&2
  exit 1
fi
...
# Safe bounds must remain enforced by optimized binary.
...
# SQL parameter placeholders are real contracts and intentionally excluded.
...
# Release defaults must request C optimizer flags (-O3 -flto). MAKO_NO_LTO is
# an explicit opt-out and must not reuse the default-LTO cache entry.
OPT_CACHE="$CACHE/release-optimization"
MAKO_CACHE="$OPT_CACHE" MAKO_CACHE_LOG=1 "$MAKO" build --release --backend c \
  "$ROOT/examples/bench/micro.mko" -o "$TMP/release-default" >"$TMP/release-default.log" 2>&1
MAKO_CACHE="$OPT_CACHE" MAKO_CACHE_LOG=1 MAKO_NO_LTO=1 "$MAKO" build --release --backend c \
  "$ROOT/examples/bench/micro.mko" -o "$TMP/release-no-lto" >"$TMP/release-no-lto.log" 2>&1
if ! rg -q "object MISS" "$TMP/release-no-lto.log"; then
  echo "claims-gate: MAKO_NO_LTO reused default-LTO artifact" >&2
