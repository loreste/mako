...
/* Sign a JWT payload with ES256 and caller-supplied protected header JSON.
 * The header is not modified; callers can set fields like ppt/typ/x5u. */
static inline MakoString mako_jwt_sign_es256_header(
    ...
    if (!payload.data || payload.len > 6000 || !private_key_pem.data
        || private_key_pem.len == 0 || private_key_pem.len > 16384
        || !header_json.data || header_json.len == 0 || header_json.len > 4096)
        return mako_str_from_cstr("");

    char alg[16];
    int has_alg = 0;
    if (mako_jwt_json_field_string(
            header_json.data, header_json.len, "alg", alg, sizeof(alg), &has_alg) != 0
