...
- `crew.kick(f(ch, …))` RC-clones channel handles (atomic retain, not a
  ...
  spawn (not Send; a deep copy would be O(n)). Coverage:
  `kick_chan_retain_test.mko`.
...
- Treat struct/enum array append as a consuming ownership boundary for
  identifier elements; fresh temporaries transfer directly.
- Clone existing element fields only during COW detach, eliminating insertion
  leaks without restoring source-alias use-after-free.
...
- Normalized `strings.fields_fn` predicate results through a typed bool local so
  native callback ABI differences cannot corrupt separator detection.
